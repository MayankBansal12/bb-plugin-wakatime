...
function hourTicks(maxMs: number): { ticks: number[]; max: number } {
  ...
  const step = TICK_STEPS.find((candidate) => maxMs / candidate <= 4) ?? TICK_STEPS.at(-1)!;
    ...
    <section className={cn("bg-card border-border flex min-w-0 flex-col rounded-xl border p-4", className)}>
      {title ? (
        <div className="mb-3 flex items-baseline justify-between gap-3">
          <h2 className="text-foreground text-[13px] leading-none font-medium">{title}</h2>
          {hint ? <span className="text-muted-foreground shrink-0 text-[11px] leading-none">{hint}</span> : null}
        </div>
      ) : null}
      {children}
    </section>
...
function Metric({ label, value, unit, detail }: {
  ...
  return (
    <div className="bg-card border-border min-w-0 rounded-xl border px-3.5 py-3">
      <p className="text-muted-foreground truncate text-[11px] leading-none">{label}</p>
      <p className="text-foreground mt-2 truncate text-xl leading-none font-semibold tracking-tight tabular-nums">
        {value}
        {unit ? <span className="text-muted-foreground ml-0.5 text-sm font-normal">{unit}</span> : null}
      </p>
      {detail ? <p className="text-muted-foreground mt-1.5 truncate text-[11px] leading-none">{detail}</p> : null}
    </div>
  );
}
...
function ContributionGraph({ days, timezone }: { days: Day[]; timezone: string }) {
  ...
  const [hovered, setHovered] = useState<{ date: string; workingMs: number; turnCount: number } | null>(null);

  const { weeks, months, thresholds, activeDays, totalMs } = useMemo(() => {
    const byDate = new Map(days.map((day) => [day.date, day]));

    // Trailing window ends on the most recent Saturday so every column is a full week.
    ...
    for (let week = 0; week < HEATMAP_WEEKS; week += 1) {
      const column: Array<{ date: string; workingMs: number; turnCount: number; future: boolean }> = [];
      ...
      const firstOfColumn = new Date(start + week * 7 * DAY_MS);
      if (firstOfColumn.getMonth() !== lastMonth) {
        lastMonth = firstOfColumn.getMonth();
        monthLabels.push({
          index: week,
          label: new Intl.DateTimeFormat(undefined, { month: "short" }).format(firstOfColumn),
        });
      }
      cells.push(column);
    }

    // Quartiles over active days only, so a handful of busy days do not flatten the rest.
    const worked = days.map((day) => day.workingMs).filter((value) => value > 0).sort((a, b) => a - b);
    const at = (fraction: number) => worked.length === 0 ? 0 : worked[Math.min(worked.length - 1, Math.floor(worked.length * fraction))]!;
    return {
      weeks: cells,
      months: monthLabels,
      thresholds: [at(0.25), at(0.5), at(0.75)],
      activeDays: worked.length,
      totalMs: worked.reduce((sum, value) => sum + value, 0),
    };
  }, [days]);
...
[data-wk-heat="${scopeId}"] {
  ...
  --wk-l1: ${PALETTE.ramp.light[0]};
  --wk-l2: ${PALETTE.ramp.light[1]};
  --wk-l3: ${PALETTE.ramp.light[2]};
  --wk-l4: ${PALETTE.ramp.light[3]};
}
.dark [data-wk-heat="${scopeId}"] {
  --wk-l0: color-mix(in oklch, var(--ink) 13%, var(--canvas));
  --wk-l1: ${PALETTE.ramp.dark[0]};
  --wk-l2: ${PALETTE.ramp.dark[1]};
  --wk-l3: ${PALETTE.ramp.dark[2]};
  --wk-l4: ${PALETTE.ramp.dark[3]};
}`;

  const caption = hovered
    ? `${formatDuration(hovered.workingMs)} on ${formatDate(hovered.date, true)}${hovered.turnCount > 0 ? ` · ${hovered.turnCount} turns` : ""}`
    : `${formatDuration(totalMs)} across ${activeDays} active day${activeDays === 1 ? "" : "s"}`;

  return (
    <div data-wk-heat={scopeId}>
      <style dangerouslySetInnerHTML={{ __html: css }} />
      <div className="mb-3 flex items-baseline justify-between gap-3">
        <h2 className="text-foreground text-[13px] leading-none font-medium">Every day bb worked</h2>
        <span className="text-muted-foreground shrink-0 text-[11px] leading-none tabular-nums">{caption}</span>
      </div>
      ...
      <div className="overflow-x-auto pb-1">
        <div className="w-max">
          <div className="mb-1 flex gap-[3px] pl-[26px]">
            {weeks.map((_, index) => {
              const month = months.find((entry) => entry.index === index);
              return (
                <span key={index} className="text-muted-foreground w-[10px] text-[10px] leading-none">
                  {month && index < HEATMAP_WEEKS - 3 ? month.label : ""}
                </span>
              );
            })}
          </div>
          ...
          <div className="flex gap-[3px]">
            <div className="mr-1 flex w-[22px] flex-col gap-[3px]">
              {["", "Mon", "", "Wed", "", "Fri", ""].map((label, index) => (
                <span key={index} className="text-muted-foreground h-[10px] text-[10px] leading-[10px]">{label}</span>
              ))}
            </div>
            {weeks.map((column, weekIndex) => (
              <div key={weekIndex} className="flex flex-col gap-[3px]">
                {column.map((cell) => {
                  const level = levelOf(cell.workingMs);
                  return cell.future ? (
                    <span key={cell.date} className="size-[10px]" />
                  ) : (
                    <span
                      key={cell.date}
                      title={`${formatDate(cell.date, true)}: ${formatDuration(cell.workingMs)}`}
                      onMouseEnter={() => setHovered(cell)}
                      onFocus={() => setHovered(cell)}
                      tabIndex={-1}
                      className="size-[10px] rounded-[2px] ring-offset-1 transition-[box-shadow] duration-100 ease-out hover:ring-1 hover:ring-current"
                      style={{ backgroundColor: `var(--wk-l${level})` }}
                    />
                  );
                })}
              </div>
      ...
      <div className="text-muted-foreground mt-3 flex items-center justify-between gap-3 text-[11px] leading-none">
        <span>{timezone}</span>
        <span className="flex items-center gap-1.5">
          less
          {[0, 1, 2, 3, 4].map((level) => (
            <span key={level} className="size-[10px] rounded-[2px]" style={{ backgroundColor: `var(--wk-l${level})` }} />
          ))}
          more
        </span>
      </div>
...
function Leaderboard({ rows, emptyLabel }: {
  ...
  return (
    <div style={{ height: Math.max(96, data.length * 30 + 8) }} className="my-auto w-full">
      <EvilBarChart
        data={data}
        config={barConfig}
        layout="horizontal"
    ...
    <dl className="grid grid-cols-2 gap-x-4 gap-y-3">
      {items.map((item) => (
        <div key={item.label} className="min-w-0">
          <dt className="text-muted-foreground truncate text-[11px] leading-none">{item.label}</dt>
          <dd className="text-foreground mt-1.5 truncate text-[13px] leading-none tabular-nums">
            {item.value}
            {item.hint ? <span className="text-muted-foreground ml-1 text-[11px]">{item.hint}</span> : null}
          </dd>
        </div>
      ))}
    </dl>
      ...
      <div className="mx-auto w-full max-w-4xl space-y-3 p-4 md:p-5">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-foreground text-base leading-none font-medium tracking-tight">Activity</h1>
            <p className="text-muted-foreground mt-1.5 text-xs leading-none">How hard bb has been working.</p>
          </div>
          <div
          ...
          >
            {RANGES.map((option) => (
              <button
                ...
                className={cn(
                  "rounded-[6px] px-2.5 py-1.5 text-xs leading-none transition-colors duration-150 ease-out active:scale-[0.97]",
                  range === option.key
        ...
        {data ? (
          <div className="space-y-3" aria-busy={refreshing}>
            ...
            <section className="bg-card border-border flex flex-wrap items-end justify-between gap-x-6 gap-y-3 rounded-xl border px-5 py-4">
              <div className="min-w-0">
                <p className="text-muted-foreground text-xs leading-none">bb worked {rangeBlurb}</p>
                <p className="text-foreground mt-2.5 flex items-baseline gap-1 text-[40px] leading-none font-semibold tracking-tight tabular-nums">
                  {heroParts.map((part) => (
                    <span key={part.unit}>
                      {part.value}
              ...
              <p className="text-muted-foreground text-xs leading-relaxed">
                {data.turnCount.toLocaleString()} turns · {data.projects.length} project{data.projects.length === 1 ? "" : "s"}
                {topAgent ? <> · <span className="text-foreground">{topAgent.name}</span> did the most</> : null}
              </p>
            </section>
            ...
            <Card title="Daily activity" hint="working time">
              <DailyChart days={data.days} />
            </Card>

            <div className="grid gap-3 md:grid-cols-2">
              <Card title="Busiest agents" hint="agent time">
                <Leaderboard rows={agents.slice(0, 5)} emptyLabel="No agent turns attributed yet." />
              </Card>
              <Card title="Projects" hint="working time">
                <Leaderboard
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Card title="Top models" hint="agent time">
                      ...
                      .map((row) => (
                        <li key={`${row.providerId}-${row.model}`} className="flex items-center justify-between gap-3 py-2">
                          <div className="flex min-w-0 items-center gap-2">
                            <span className="text-foreground truncate text-[13px]">{shortModel(row.model)}</span>
                            <span className="text-muted-foreground shrink-0 text-[11px]">{providerLabel(row.providerId)}</span>
                          </div>
                          <span className="text-foreground shrink-0 text-[13px] tabular-nums">{formatDuration(row.agentRuntimeMs)}</span>
                        </li>
                      ))}
                  </ul>
                )}
            ...
            {data.machines.length > 0 ? (
              <Card title="Machines" hint="working time">
                <ul className="flex flex-wrap gap-2">
                  {data.machines.slice(0, 6).map((row) => (
                    ...
                    >
                      ...
                      />
                      <span className="text-foreground truncate text-[13px]">{row.name}</span>
